    ...
    public function store(Request $request)
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $validatedData = $request->validate([
            ...
            'system' => 'nullable|string',
            'power_suply' => 'nullable|string', // Menambahkan validasi untuk power_suply
            'status' => 'required|string',
            'front_port' => 'nullable|string',
            'display_size' => 'nullable|string',
            'product_size' => 'nullable|string',
            ...
            'quantity' => 'required|integer',
            'rating' => 'nullable|integer|min:0|max:5',
            'price' => 'nullable|numeric', // Menambahkan validasi untuk price
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
        ...
        $product->update([
            'name' => $validatedData['name'],
            'color' => $validatedData['color'],
            'category' => $validatedData['category'], // Simpan kategori sebagai string
            'system' => $validatedData['system'],
            'power_suply' => $validatedData['power_suply'], // Menyimpan power_suply
            'status' => $validatedData['status'],
            'front_port' => $validatedData['front_port'],
            'display_size' => $validatedData['display_size'],
            'product_size' => $validatedData['product_size'],
            ...
            'quantity' => $validatedData['quantity'],
            'rating' => $validatedData['rating'],
            'price' => $validatedData['price'], // Menyimpan price
            'image' => $validatedData['image'] ?? null,
        ]);
